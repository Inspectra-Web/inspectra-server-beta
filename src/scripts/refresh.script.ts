 ...
 */
const backfillRefreshed = async (): Promise<void> => {
  await dbConfig();

  const dated = await Property.updateMany(
    { refreshedAt: trusted({ $exists: false }) },
    [{ $set: { refreshedAt: "$createdAt", hiddenByPlan: false } }],
...
backfillRefreshed().catch(async (error: unknown) => {

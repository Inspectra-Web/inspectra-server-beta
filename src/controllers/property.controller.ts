...
export const getProperty = async (req: Request, res: Response): Promise<void> => {
  ...
  const privileged =
    !!req.user && (req.user.role === "admin" || property.user.equals(req.user._id));
 ...
 */
const assertListingRoom = async (user: UserDoc): Promise<void> => {
  ...
  throw new AppError(
    `You have used all ${limit} listing${limit === 1 ? "" : "s"} on your ${plan} plan. ` +
      `Upgrade, or mark a listing sold, to add another.`,
    403,
  );
};
 ...
 */
export const refreshMyProperty = async (req: Request, res: Response): Promise<void> => {
  const { id } = propertyIdSchema.parse(req.params);
  const user = req.user!;
  ...
  if (left <= 0)
    throw new AppError(
      allowed === 0
        ...
        : "You have used every refresh on your plan this month.",
      403,
    );

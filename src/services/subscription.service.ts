...
export const entitlements = (subscription: SubscriptionDoc): Plan =>
 ...
 */
export const syncHiddenListings = async (
  ...
  if (overflow.length)
    await Property.updateMany(
      { _id: trusted({ $in: overflow }) },
      { $set: { hiddenByPlan: true } },
    );
};
/**
 ...
 */
const advance = async (subscription: SubscriptionDoc): Promise<SubscriptionDoc> => {
  const now = new Date();
  let changed = false;
  let lapsed = false;
  ...
  if (
    subscription.refreshPeriodStart &&
  ...
  if (changed) await subscription.save({ validateModifiedOnly: true });
  if (lapsed) await syncHiddenListings(subscription.user, subscription);
...
export const resolveSubscription = async (
  ...
  if (existing) return advance(existing);
 ...
 */
export const listingAllowance = async (
  userId: Types.ObjectId,
  subscription: SubscriptionDoc,
): Promise<ListingAllowance> => {
  const limit = entitlements(subscription).listings;

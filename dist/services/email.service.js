...
const adminListing = (listing, subject, lead) => notify(listing.ref, async () => {
...
export const sendListingSubmitted = (listing) => adminListing(listing, `New listing to verify: ${listing.title}`, "A realtor submitted a listing. It is waiting on verification.");
/**
...
export const sendListingUpdated = (listing, recheck) => adminListing(listing, recheck
    ...
    : `Listing updated: ${listing.title}`, recheck
...
export const sendListingReviewed = (to, listing, status, note, flagged) => {
    ...
    notify(listing.ref, () => sendEmail({ to, subject: verdict.subject(listing.title), body }));
};
/**

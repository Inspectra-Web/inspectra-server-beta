...
const send = (buffer, folder, resourceType) => new Promise((resolve, reject) => {
...
const fade = (image, alpha) => sharp(image)
    .composite([
    {
...
const buildMark = async (width) => {
    ...
    // dark interior.
    const shadow = await fade(await sharp(logo).negate({ alpha: false }).png().toBuffer(), SHADOW_OPACITY * MARK_OPACITY);
...
export const uploadPhoto = async (buffer) => {
    ...
    const mark = await markFor(Math.min(info.width, Math.max(MARK_MIN, Math.round(info.width * MARK_RATIO))));
...
export const uploadDocument = async (buffer) => send(buffer, DOCUMENT_FOLDER, "auto");
/**
 ...
 */
export const uploadBill = async (buffer) => {
    if (isPdf(buffer))
        return send(buffer, BILL_FOLDER, "image");
    const image = await sharp(buffer)
        .rotate()
...
export const destroyAvatar = (publicId) => destroyAsset(publicId);

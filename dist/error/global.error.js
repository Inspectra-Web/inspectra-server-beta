...
import { AVATAR_MAX_MB, DOCUMENT_MAX_MB, PHOTO_MAX_MB, } from "../services/upload.service.js";
...
const handleZodError = (error) => {
    ...
    const messages = error.issues.map((issue) => issue.path.length ? `${issue.path.join(".")}: ${issue.message}` : issue.message);
    return new AppError(messages.join(". "), 422);
};

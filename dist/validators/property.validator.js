 ...
 */
export const typeFitsCategory = (category, type) => PROPERTY_TYPES_BY_CATEGORY[category].includes(type);
const checkPair = (body, ctx) => {
    if (!body.category || !body.type)
        return;
    if (typeFitsCategory(body.category, body.type))
        return;
    ctx.addIssue({
        code: "custom",
        path: ["type"],
...
const STATUS_FILTERS = ["all", ...VERIFICATION_STATUSES];
const LISTING_FILTERS = ["all", ...LISTING_STATUSES];
const TYPE_FILTERS = ["all", ...PROPERTY_TYPES];
const CATEGORY_FILTERS = ["all", ...PROPERTY_CATEGORIES];
/**

...
import Inquiry, { inquiryThread, } from "../models/inquiry.model.js";
...
const findOwn = async (id, side, userId) => {
    ...
    if (!inquiry)
        throw new AppError("No conversation with that id.", 404);
 ...
 */
const threadContext = async (inquiry, side) => {
...
const loadInquiries = async (side, userId, query) => {
...
const brief = (inquiry, property, person, message) => ({
...
export const createInquiry = async (req, res) => {
    ...
    sendInquiryReceived(owner.email, brief(inquiry, property, seeker.fullname, message), !existing);
...
export const updateLeadStatus = async (req, res) => {
    ...
    const reopened = last && inquiry.seeker.equals(last.sender) ? "new" : "responded";
    const next = status === "closed" ? "closed" : reopened;
    if (inquiry.status === next)
        throw new AppError(status === "closed"
            ? "This conversation is already closed."
            : "This conversation is already open.", 409);
    inquiry.status = next;
...
export const addInquiryMessage = async (req, res) => {
    ...
    if (counterpart && property) {
        ...
        if (fromSeeker)
            sendInquiryReceived(counterpart.email, note, false);
        else
            sendInquiryReplied(counterpart.email, note);
    }
    res.status(201).json({
        status: "success",

...
import Agency, { publicAgency } from "../models/agency.model.js";
 ...
 */
const verifiedIdentity = async (user, what) => {
...
const cacCheck = async (rcNumber, companyType) => {
    const query = new URLSearchParams({ rc_number: rcNumber, company_type: companyType });
    const response = await fetch(`${envConfig.DOJAH_BASE_URL}/api/v1/kyc/cac/advance?${query.toString()}`, { headers: dojahHeaders });
    // The type is part of the lookup, so the wrong pick misses a real company. Name both.
    if (response.status === 404)
        throw new AppError("We could not find that number under that registration type. Check both.", 404);
    if (!response.ok)
...
export const getMyAgency = async (req, res) => {
    const agency = await Agency.findOneAndUpdate({ user: req.user._id }, {}, { upsert: true, returnDocument: "after", runValidators: true });
    res.status(200).json({ status: "success", data: { agency: publicAgency(agency) } });
};
export const verifyMyCac = async (req, res) => {
    ...
    if (affiliates.length === 0)
        throw new AppError("That registration lists no directors we can match you to.", 400);
    const mine = affiliates.some((affiliate) => namesMatch([...words(affiliate.first_name ?? ""), ...words(affiliate.last_name ?? "")], identity.legalName));
    if (!mine)
        throw new AppError("Your name is not listed on that registration.", 400);
    const agency = await Agency.findOneAndUpdate({ user: user._id }, {
        cac: {
...
export const submitMyAddress = async (req, res) => {
    const { meterNumber } = req.body;
    const user = req.user;
    if (!req.file)
        throw new AppError("Add your utility bill.", 400);

...
const tierOf = (value) => TIERS.find((t) => t === value) ?? null;
const cadenceOf = (value) => CADENCES.find((c) => c === value) ?? null;
/**
 ...
 */
const applyToSubscription = async (payment) => {
    const subscription = await resolveSubscription(payment.user);
    if (!payment.tier || !payment.cadence || !payment.periodEnd)
        return subscription;
...
const creditPayment = async (payment, flw) => {
    ...
    const renewing = subscription.tier === payment.tier &&
        !!subscription.currentPeriodEnd &&
        subscription.currentPeriodEnd > now;
    const period = periodFor(payment.cadence, renewing && subscription.currentPeriodEnd ? subscription.currentPeriodEnd : now);
    ...
    if (!claimed) {
        ...
        if (settled)
            await applyToSubscription(settled);
...
const settledPayload = async (payment, userId) => {
    ...
    if (event === "charge.completed" && data?.id && data.tx_ref) {
        ...
        try {
            ...
            if (payment && payment.status === "pending")
                await creditPayment(payment, flw);
        }
        catch (error) {

...
export const listUsers = async (req, res) => {
    ...
    const roleMatch = role === "all" ? [] : [{ $match: { role } }];
    ...
    }, { $unwind: { path: "$profile", preserveNullAndEmptyArrays: true } }, { $addFields: { city: { $ifNull: ["$profile.city", ""] } } });
    if (q) {
 ...
 */
export const reviewRealtorAddress = async (req, res) => {
...
export const listRealtors = async (req, res) => {
    ...
    const statusMatch = status === "all" ? [] : [{ $match: { status } }];
    const pipeline = [{ $match: { role: "realtor" } }];
    pipeline.push({
        $lookup: {
            from: "profiles",
            localField: "_id",
    ...
    const certifiedMatch = certified === "all" ? [] : [{ $match: { certified: certified === "yes" } }];
...
export const reviewListing = async (req, res) => {
    const { id } = listingIdSchema.parse(req.params);
    const { status, note, documents } = req.body;
    const property = await Property.findById(id);
    if (!property)
        throw new AppError("No listing with that id.", 404);
    for (const decision of documents) {
        ...
        if (!doc)
            throw new AppError("That document is not on this listing.", 422);
    ...
    if (!property.isModified())
        throw new AppError("Nothing changed in this review.", 409);
    ...
    if (owner)
        sendListingReviewed(owner.email, {
        ...
        }, status, note, property.documents

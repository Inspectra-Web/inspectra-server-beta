...
import Inspection, { ACTIVE_STATUSES, inspectionRecord, } from "../models/inspection.model.js";
...
const live = (status) => ACTIVE_STATUSES.includes(status);
/**
 ...
 */
const findOwn = async (id, side, userId) => {
 ...
 */
const bookingContext = async (inspection, side) => {
...
const loadInspections = async (side, userId, query) => {
    ...
    const scope = [];
    if (window !== "all")
        scope.push({ $match: { bucket: window } });
    if (status !== "all")
        scope.push({ $match: { status } });
    pipeline.push({
        $facet: {
            rows: [
                ...scope,
...
const brief = (inspection, property, person, message) => ({
...
export const createInspection = async (req, res) => {
    const { property: propertyId, slot, note } = req.body;
    ...
    if (existing)
        throw new AppError("You already have a viewing booked on this listing. Move that one rather than booking a second.", 409);
    ...
    sendInspectionRequested(owner.email, brief(inspection, property, seeker.fullname, inspection.note));
    res.status(201).json({
        status: "success",
...
export const listMyInspections = async (req, res) => {
...
export const getMyInspection = async (req, res) => {
...
export const rescheduleInspection = async (req, res) => {
    ...
    if (property && realtor)
        sendInspectionRescheduled(realtor.email, brief(inspection, property, req.user.fullname, ""));
    res.status(200).json({
        status: "success",
 ...
 * ------------------------------------------------------------------ */
export const listRealtorInspections = async (req, res) => {
    const query = listInspectionsSchema.parse(req.query);
...
export const getRealtorInspection = async (req, res) => {
    const { id } = inspectionIdSchema.parse(req.params);
...
export const decideInspection = async (req, res) => {
    const { id } = inspectionIdSchema.parse(req.params);
    ...
    if (property && seeker)
        sendInspectionDecided(seeker.email, brief(inspection, property, req.user.fullname, inspection.response), status);
    res.status(200).json({
        status: "success",
...
export const cancelInspection = async (req, res) => {
    ...
    if (property && counterpart)
        sendInspectionCancelled(counterpart.email, brief(inspection, property, user.fullname, ""), side);
    res.status(200).json({
        status: "success",
